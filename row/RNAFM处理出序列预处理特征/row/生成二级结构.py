...
def calculate_structure_features(sequence, structure):
    ...
    try:
        ...
        try:
            ...
            features['centroid_distance'] = RNA.bp_distance(structure, centroid_struct)

            # 集合多样性
            features['ensemble_diversity'] = fc.mean_bp_distance()

            # 配分函数自由能
            features['ensemble_energy'] = fc.pf()[1]  # 配分函数自由能

        except Exception as ve:
            # 如果高级特征计算失败，只计算基本特征
            features['centroid_distance'] = 0.0
            features['ensemble_diversity'] = 0.0
            features['ensemble_energy'] = 0.0
