...
def read_unmatched_ids(unmatched_file):
    ...
    unmatched_ids = set(df['ENSEMBL_ID'].tolist())
    return unmatched_ids


def filter_target_file(target_file, unmatched_ids, id_column='ENSEMBL_ID'):
    """
    过滤目标文件，删除在未匹配列表中的行
    """
    # 读取目标文件
...
def main():
    # 文件路径设置
    unmatched_file = r'D:\Desktop\CDLLM\ing\row\删除不存在的lncRNA\需要删除的lncRNA.xlsx'  # 未匹配ID文件
    target_file = r'D:\Desktop\CDLLM\ing\row\删除不存在的lncRNA\RNAInter第一步收集.xlsx'  # 需要过滤的目标文件
